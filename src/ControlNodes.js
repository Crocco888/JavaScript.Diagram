...
var Utils = MindFusion.Controls.DomUtils;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    diagram.addEventListener("nodeCreating", (sender, args) => {
        args.cancel = divtarget;
    });

    diagram.addEventListener("nodeDomCreated", function (sender, args) {
        var node = args.node;
        var checkbox = node.content.querySelector("input[type='checkbox']");
        if (checkbox) {
            checkbox.checked = node.tag;
            node.createImage();
        }
    });
    ...
    nodeList.defaultNodeSize = new MindFusion.Drawing.Size(20, 40);

    nodeList.addEventListener("nodeSelected", () => {
        divtarget = false;
    });
...
<div style="display:flex;background-color:lightcyan;">
	<img width="70" height="70" title="Click to choose picture" src="data:image/png;base64,${placeholderBase64}" data-interactive="true" data-event-click="placeholderClick" />
	<input placeholder="Text" data-interactive="true" data-event-change="dataChange"></input>
<div style="padding:10px">
	<button data-interactive="true" data-event-click="subordinateClick">Subordinate</button>
...
<div>
    ...
    Utils.loadImageList(["icon_print.png", "icon_delivery.png", "icon_calculator.png"]).then(response => {
        ...
        node.bounds = new Rect(10, 10, 100, 20);
        node.stroke = "transparent";
        node.text = "node";
        diagram.addItem(node);

        var node1 = new MindFusion.Diagramming.ControlNode(diagramView);
        node1.bounds = new Rect(10, 50, 100, 20);
        diagram.addItem(node1);

        diagram.factory.createDiagramLink(node, node1);

        var node1 = new MindFusion.Diagramming.ControlNode(diagramView);
        node1.template = `<img width="50" height="50" src="${response[0]}"></img><p>Print an invoice</p>Finished<input type="checkbox" 
data-interactive="true" data-event-change="dataChange"></input>`;
        node1.bounds = new Rect(110, 50, 20, 40)
        diagram.addItem(node1);
        diagram.factory.createDiagramLink(diagram.nodes[0], node1);
        arrange();
        nodeList.addNode(node1.clone(), "Print");

        var node2 = new MindFusion.Diagramming.ControlNode(diagramView);
        node2.template = `<img width="50" height="50" src="${response[1]}"></img><p>Deliver products</p>Finished<input type="checkbox" 
data-interactive="true" data-event-change="dataChange"></input>`;
        nodeList.addNode(node2, "Delivery");

        var node3 = new MindFusion.Diagramming.ControlNode(diagramView);
        node3.template = `<img width="50" height="50" src="${response[2]}"></img><p>Calculate prices</p>Finished<input type="checkbox" 
data-interactive="true" data-event-change="dataChange"></input>`;
        nodeList.addNode(node3, "Calculation");
    });
});

function dataChange(e, sender) {
    if (e.target.type == "checkbox")
        sender.tag = e.target.checked;
    else
        sender.text = e.target.value;
    sender.createImage();
}
...
function tasksMouseUp(e, sender) {
    divtarget = true;
    var nodeList = NodeListView.find("nodeList");
    var node = nodeList.draggedNode;
    if (node) {
        var clone = node.clone();
        var bounds = sender.bounds.clone();
        bounds.x += bounds.width + 10;
        bounds.width = 20;
        bounds.height = 40;
        clone.bounds = bounds;
        sender.parent.addItem(clone);
        var link = sender.parent.factory.createDiagramLink(sender, clone);
        link.headShape = null;
        arrange();
    }
    //nodeList._draggedNode = null;
}

function subordinateClick(e, sender) {
    var clone = sender.clone();
    var bounds = sender.bounds.clone();
    bounds.y += bounds.height + 10;
    clone.bounds = bounds;
    sender.parent.addItem(clone);
    sender.parent.factory.createDiagramLink(sender, clone);
    arrange();
}

function deleteClick(e, sender) {
...
function arrange() {
    ...
    // arrange the diagram as a tree
    var tree = new MindFusion.Graphs.TreeLayout();
    ...
    var treeBounds = diagram.getContentBounds();
    diagram.bounds = diagram.bounds.union(treeBounds);
}
